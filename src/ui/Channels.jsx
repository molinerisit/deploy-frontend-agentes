...
function pickQrDataUrl(r) {
  const cands = [
    ...
    // algunas respuestas anidan en body/response
    r?.body?.qr, r?.body?.dataUrl, r?.response?.qr, r?.response?.dataUrl
  ];
...
function pickLinkCode(r) {
  const cands = [
    r?.code, r?.linkCode, r?.link, r?.loginCode,
    r?.body?.code, r?.response?.code
  ];
  for (const v of cands) {
...
function pickPairingCode(r) {
  const cands = [
    r?.pairingCode, r?.pairing, r?.pin, r?.code_short,
    r?.body?.pairingCode, r?.response?.pairingCode
  ];
  for (const v of cands) {
...
// Genera un src de imagen QR con servicio público si no tenemos data URL.
// (Si querés 100% self-hosted, cambiamos por una lib local de QR.)
...
export default function Channels({ api, brands, brandId, setBrandId }) {
  ...
  const forceWebhook = async () => {
    ...
    try {
      ...
      console.log("[WA] set_webhook result:", r);
      if (r?.ok) setStatus("Webhook forzado (ver logs del backend).");
      else setErr("No se pudo setear el webhook (ver detalle en logs).");
    } catch (e) {
  ...
  const syncPull = async () => {
    ...
    try {
      ...
      setStatus(`Sync (pull) OK — guardados ${r?.saved ?? 0} mensajes.`);
    } catch (e) {
  ...
  return (
    <div className="card">
      ...
      <div className="row">
        ...
        {selectedBrand && (
          <div className="hint">Marca: <b>{selectedBrand.name}</b></div>
        )}
      </div>
  ...
  return (
    <div className="row">
      ...
      <input
        ...
        value={fbImg}
        onChange={(e) => setFbImg(e.target.value)}
      />
      <button className="btn" disabled={busy} onClick={postFB}>
        Publicar FB
      </button>
    </div>
  );
  ...
  return (
    <div className="row">
      <input
        ...
        placeholder="Caption"
        value={igCap}
        onChange={(e) => setIgCap(e.target.value)}
      />
      <input
        className="input grow"
        placeholder="URL Imagen"
        value={igImg}
        onChange={(e) => setIgImg(e.target.value)}
      />
      <button className="btn" disabled={busy} onClick={postIG}>
        Publicar IG
      </button>
    </div>
  );
