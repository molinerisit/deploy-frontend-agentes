...
export default function App() {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      try { await api('/api/health'); if (mounted) setHealth('✅'); }
      catch { if (mounted) setHealth('❌'); }
      try {
  ...
  return (
    <div className="app">
      <aside className="aside">
        ...
        <div className="meta">
          <div>API: <code>{API_BASE}</code></div>
          <div>Backend: <span className={`badge ${health==='✅'?'ok':health==='❌'?'bad':'wait'}`}>{health}</span></div>
        </div>
        <nav className="tabs">
          {TABS.map(t => (
